...
<div id="currency" class="header-button">
	<div>
		<div class="heading-1"><i class="icon-money"></i><?php echo $text_currency; ?><i class="icon-angle-down"></i></div>
		<div class="heading">
		<?php foreach ($currencies as $currency) { ?>
			<?php if ($currency['code'] == $currency_code) { ?>
			<?php if ($currency['symbol_left']) { ?>
			<?php echo $currency['symbol_left']; ?><i class="icon-angle-down"></i>
			<?php } else { ?>
			<?php echo $currency['symbol_right']; ?><i class="icon-angle-down"></i>
			<?php } ?>
			<?php } ?>
		<?php } ?>
		</div>
